import require$$0$5, { existsSync, readdirSync as readdirSync$1 } from 'fs';
import require$$0$4 from 'path';
import { a as commonjsGlobal, g as commonjsRequire, d as defineNuxtCommand, c as consola } from './index.mjs';
...
import { r as rimraf_1$1 } from './rimraf.mjs';
import { r as resolve$1, a as relative } from './index2.mjs';
...
class ZlibBase extends Minipass {
  ...
  reset () {
    if (!this[_sawError]) {
      assert$3(this[_handle], 'zlib binding closed');
...
class Pax$2 {
  ...
  encode () {
    ...
    new Header$3({
      ...
      path: ('PaxHeader/' + path$d.basename(this.path)).slice(0, 99),
...
const WriteEntry$1 = warner$2(class WriteEntry extends MiniPass$2 {
  constructor (p, opt) {
    ...
    let pathWarn = false;
    if (!this.preservePaths && path$c.win32.isAbsolute(p)) {
      // absolutes on posix are also absolutes on win32
      // so we only need to test this one to get both
      const parsed = path$c.win32.parse(p);
    ...
    this.absolute = opt.absolute || path$c.resolve(this.cwd, p);
  ...
  [LSTAT] () {
    fs$g.lstat(this.absolute, (er, stat) => {
      if (er)
  ...
  [SYMLINK$1] () {
    fs$g.readlink(this.absolute, (er, linkpath) => {
      if (er)
        return this.emit('error', er)
  ...
  [HARDLINK$1] (linkpath) {
    this.type = 'Link';
    this.linkpath = path$c.relative(this.cwd, linkpath).replace(/\\/g, '/');
    this.stat.size = 0;
    this[HEADER]();
    this.end();
  }
  ...
  [OPENFILE] () {
    fs$g.open(this.absolute, 'r', (er, fd) => {
      if (er)
        return this.emit('error', er)
      this[ONOPENFILE](fd);
    });
  ...
  [READ] (fd, buf, offset, length, pos, remain, blockRemain) {
    fs$g.read(fd, buf, offset, length, pos, (er, bytesRead) => {
      if (er) {
...
class WriteEntrySync$1 extends WriteEntry$1 {
  [LSTAT] () {
    this[ONLSTAT](fs$g.lstatSync(this.absolute));
  }

  [SYMLINK$1] () {
    this[ONREADLINK](fs$g.readlinkSync(this.absolute));
  }

  [OPENFILE] () {
    this[ONOPENFILE](fs$g.openSync(this.absolute, 'r'));
  }

  [READ] (fd, buf, offset, length, pos, remain, blockRemain) {
    let threw = true;
    try {
      const bytesRead = fs$g.readSync(fd, buf, offset, length, pos);
      this[ONREAD](fd, buf, offset, length, pos, remain, blockRemain, bytesRead);
      threw = false;
    } finally {
      // ignoring the error from close(2) is a bad practice, but at
...
const WriteEntryTar$1 = warner$2(class WriteEntryTar extends MiniPass$2 {
  constructor (readEntry, opt) {
    ...
    let pathWarn = false;
    if (path$c.isAbsolute(this.path) && !this.preservePaths) {
      const parsed = path$c.parse(this.path);
      pathWarn = parsed.root;
...
const Pack$2 = warner$1(class Pack extends MiniPass$1 {
  ...
  [ADDTARENTRY] (p) {
    const absolute = path$b.resolve(this.cwd, p.path);
    if (this.prefix)
  ...
  [ADDFSENTRY] (p) {
    const absolute = path$b.resolve(this.cwd, p);
    if (this.prefix)
  ...
  [STAT] (job) {
    ...
    fs$f[stat](job.absolute, (er, stat) => {
      job.pending = false;
      this[JOBS] -= 1;
      if (er)
        this.emit('error', er);
  ...
  [READDIR] (job) {
    job.pending = true;
    this[JOBS] += 1;
    fs$f.readdir(job.absolute, (er, entries) => {
      job.pending = false;
      this[JOBS] -= 1;
      if (er)
        return this.emit('error', er)
...
class PackSync extends Pack$2 {
  ...
  [STAT] (job) {
    ...
    this[ONSTAT](job, fs$f[stat](job.absolute));
  }

  [READDIR] (job, stat) {
    this[ONREADDIR](job, fs$f.readdirSync(job.absolute));
  }
...
class ReadStream extends MiniPass {
  ...
  [_open] () {
    fs$e.open(this[_path], 'r', (er, fd) => this[_onopen](er, fd));
  }
  ...
  [_read] () {
    if (!this[_reading]) {
      ...
      fs$e.read(this[_fd], buf, 0, buf.length, null, (er, br, buf) =>
  ...
  [_close] () {
    if (this[_autoClose] && typeof this[_fd] === 'number') {
      const fd = this[_fd];
      this[_fd] = null;
      fs$e.close(fd, er => er ? this.emit('error', er) : this.emit('close'));
    }
  }
...
class ReadStreamSync extends ReadStream {
  [_open] () {
    let threw = true;
    try {
      this[_onopen](null, fs$e.openSync(this[_path], 'r'));
      threw = false;
    } finally {
      if (threw)
        this[_close]();
      ...
      if (!this[_reading]) {
        ...
        do {
          ...
          const br = buf.length === 0 ? 0
            : fs$e.readSync(this[_fd], buf, 0, buf.length, null);
...
class WriteStream extends EE$1 {
  ...
  [_open] () {
    fs$e.open(this[_path], this[_flags], this[_mode],
  ...
  [_write] (buf) {
    fs$e.write(this[_fd], buf, 0, buf.length, this[_pos], (er, bw) =>
      this[_onwrite](er, bw));
  }
...
class WriteStreamSync extends WriteStream {
  [_open] () {
    ...
    if (this[_defaultFlag] && this[_flags] === 'r+') {
      try {
        fd = fs$e.openSync(this[_path], this[_flags], this[_mode]);
      } catch (er) {
        if (er.code === 'ENOENT') {
          this[_flags] = 'w';
          return this[_open]()
  ...
  [_write] (buf) {
    ...
    try {
      this[_onwrite](null,
        fs$e.writeSync(this[_fd], buf, 0, buf.length, this[_pos]));
      threw = false;
    } finally {
      if (threw)
...
const filesFilter$1 = (opt, files) => {
  ...
  const mapHas = (file, r) => {
    const root = r || path$a.parse(file).root || '.';
    const ret = file === root ? false
      : map.has(file) ? map.get(file)
      : mapHas(path$a.dirname(file), root);
...
const listFileSync = opt => {
  ...
  let fd;
  try {
    const stat = fs$d.statSync(file);
    const readSize = opt.maxReadSize || 16 * 1024 * 1024;
    if (stat.size < readSize)
      p.end(fs$d.readFileSync(file));
    else {
      let pos = 0;
      const buf = Buffer.allocUnsafe(readSize);
      fd = fs$d.openSync(file, 'r');
      while (pos < stat.size) {
        const bytesRead = fs$d.readSync(fd, buf, 0, readSize, pos);
        pos += bytesRead;
...
const replaceSync = (opt, files) => {
  ...
  try {
    try {
      fd = fs$c.openSync(opt.file, 'r+');
    } catch (er) {
      if (er.code === 'ENOENT')
        fd = fs$c.openSync(opt.file, 'w+');
      else
        throw er
    }

    const st = fs$c.fstatSync(fd);
  ...
  const getPos = (fd, size, cb_) => {
    ...
    const onread = (er, bytes) => {
      ...
      if (opt.mtimeCache)
        opt.mtimeCache.set(h.path, h.mtime);
      bufPos = 0;
      fs$c.read(fd, headBuf, 0, 512, position, onread);
    };
    fs$c.read(fd, headBuf, 0, 512, position, onread);
  };
  ...
  const promise = new Promise((resolve, reject) => {
    ...
    const onopen = (er, fd) => {
      if (er && er.code === 'ENOENT' && flag === 'r+') {
        flag = 'w+';
        return fs$c.open(opt.file, flag, onopen)
      }
    ...
    fs$c.open(opt.file, flag, onopen);
  });
...
const addFilesSync = (p, files) => {
  files.forEach(file => {
    if (file.charAt(0) === '@') {
      t({
        file: path$8.resolve(p.cwd, file.substr(1)),
        sync: true,
        noResume: true,
        onentry: entry => p.add(entry),
      });
...
const optsArg$1 = opts => {
  if (!opts)
    opts = { mode: 0o777, fs: fs$b };
  else if (typeof opts === 'object')
    opts = { mode: 0o777, fs: fs$b, ...opts };
  else if (typeof opts === 'number')
    opts = { mode: opts, fs: fs$b };
  else if (typeof opts === 'string')
    opts = { mode: parseInt(opts, 8), fs: fs$b };
  else
    throw new TypeError('invalid options argument')

  opts.mkdir = opts.mkdir || opts.fs.mkdir || fs$b.mkdir;
  opts.mkdirAsync = promisify$2(opts.mkdir);
  opts.stat = opts.stat || opts.fs.stat || fs$b.stat;
  opts.statAsync = promisify$2(opts.stat);
  opts.statSync = opts.statSync || opts.fs.statSync || fs$b.statSync;
  opts.mkdirSync = opts.mkdirSync || opts.fs.mkdirSync || fs$b.mkdirSync;
  return opts
};
var optsArg_1 = optsArg$1;

const platform$2 = process.env.__TESTING_MKDIRP_PLATFORM__ || process.platform;
const { resolve, parse: parse$2 } = require$$0$4;
...
const fs$a = require$$0$5;

const version = process.env.__TESTING_MKDIRP_NODE_VERSION__ || process.version;
const versArr = version.replace(/^v/, '').split('.');
const hasNative = +versArr[0] > 10 || +versArr[0] === 10 && +versArr[1] >= 12;

const useNative$1 = !hasNative ? () => false : opts => opts.mkdir === fs$a.mkdir;
const useNativeSync$1 = !hasNative ? () => false : opts => opts.mkdirSync === fs$a.mkdirSync;
...
const chownrKid = (p, child, uid, gid, cb) => {
  if (typeof child === 'string')
    return fs$9.lstat(path$7.resolve(p, child), (er, stats) => {
      // Skip ENOENT error
      if (er)
  ...
  if (child.isDirectory()) {
    chownr$1(path$7.resolve(p, child.name), uid, gid, er => {
      if (er)
        return cb(er)
      const cpath = path$7.resolve(p, child.name);
...
const chownrKidSync = (p, child, uid, gid) => {
  if (typeof child === 'string') {
    try {
      const stats = fs$9.lstatSync(path$7.resolve(p, child));
      stats.name = child;
      child = stats;
    } catch (er) {
      if (er.code === 'ENOENT')
  ...
  if (child.isDirectory())
    chownrSync(path$7.resolve(p, child.name), uid, gid);

  handleEISDirSync(path$7.resolve(p, child.name), uid, gid);
};
...
mkdir$2.exports = (dir, opt, cb) => {
  ...
  const sub = path$6.relative(cwd, dir);
...
const mkdir_ = (base, parts, mode, cache, unlink, cwd, created, cb) => {
  ...
  fs$8.mkdir(part, mode, onmkdir(part, parts, mode, cache, unlink, cwd, created, cb));
};

const onmkdir = (part, parts, mode, cache, unlink, cwd, created, cb) => er => {
  if (er) {
    if (er.path && path$6.dirname(er.path) === cwd &&
        (er.code === 'ENOTDIR' || er.code === 'ENOENT'))
      return cb(new CwdError(cwd, er.code))

    fs$8.lstat(part, (statEr, st) => {
      if (statEr)
        cb(statEr);
...
mkdir$2.exports.sync = (dir, opt) => {
  ...
  if (dir === cwd) {
    ...
    let code = 'ENOTDIR';
    try {
      ok = fs$8.statSync(dir).isDirectory();
    } catch (er) {
      code = er.code;
    } finally {
      if (!ok)
  ...
  for (let p = parts.shift(), part = cwd;
    ...
    try {
      ...
      const st = fs$8.lstatSync(part);
      if (st.isDirectory()) {
        cache.set(part, true);
        continue
      } else if (unlink) {
        fs$8.unlinkSync(part);
        fs$8.mkdirSync(part, mode);
        created = created || part;
        cache.set(part, true);
        continue
...
const fs$7 = commonjsGlobal.__FAKE_TESTING_FS__ || require$$0$5;

/* istanbul ignore next */
const { O_CREAT, O_TRUNC, O_WRONLY, UV_FS_O_FILEMAP = 0 } = fs$7.constants;
...
const unlinkFile = (path, cb) => {
  if (process.platform !== 'win32')
    return fs$6.unlink(path, cb)

  const name = path + '.DELETE.' + crypto.randomBytes(16).toString('hex');
  fs$6.rename(path, name, er => {
    if (er)
      return cb(er)
    fs$6.unlink(name, cb);
  });
...
const unlinkFileSync = path => {
  if (process.platform !== 'win32')
    return fs$6.unlinkSync(path)

  const name = path + '.DELETE.' + crypto.randomBytes(16).toString('hex');
  fs$6.renameSync(path, name);
  fs$6.unlinkSync(name);
};
...
class Unpack$1 extends Parser {
  constructor (opt) {
    ...
    this.cwd = path$5.resolve(opt.cwd || process.cwd());
  ...
  [CHECKPATH] (entry) {
    ...
    if (!this.preservePaths) {
      ...
      // absolutes on posix are also absolutes on win32
      // so we only need to test this one to get both
      if (path$5.win32.isAbsolute(p)) {
        const parsed = path$5.win32.parse(p);
    ...
    if (this.win32) {
      const parsed = path$5.win32.parse(entry.path);
      entry.path = parsed.root === '' ? wc.encode(entry.path)
        : parsed.root + wc.encode(entry.path.substr(parsed.root.length));
    }

    if (path$5.isAbsolute(entry.path))
      entry.absolute = entry.path;
    else
      entry.absolute = path$5.resolve(this.cwd, entry.path);
  ...
  [FILE] (entry, fullyDone) {
    ...
    stream.on('finish', _ => {
      ...
      if (entry.mtime && !this.noMtime) {
        ...
        fs$6.futimes(fd, atime, mtime, er =>
          er ? fs$6.utimes(abs, atime, mtime, er2 => done(er2 && er))
          : done());
      }
      ...
      if (this[DOCHOWN](entry)) {
        ...
        fs$6.fchown(fd, uid, gid, er =>
          er ? fs$6.chown(abs, uid, gid, er2 => done(er2 && er))
          : done());
      }
  ...
  [DIRECTORY] (entry, fullyDone) {
    ...
    this[MKDIR](entry.absolute, mode, er => {
      ...
      if (entry.mtime && !this.noMtime) {
        actions++;
        fs$6.utimes(entry.absolute, entry.atime || new Date(), entry.mtime, done);
      }

      if (this[DOCHOWN](entry)) {
        actions++;
        fs$6.chown(entry.absolute, this[UID](entry), this[GID](entry), done);
      }
  ...
  [HARDLINK] (entry, done) {
    this[LINK](entry, path$5.resolve(this.cwd, entry.linkpath), 'link', done);
  }
  ...
  [CHECKFS2] (entry, done) {
    this[MKDIR](path$5.dirname(entry.absolute), this.dmode, er => {
      if (er) {
        done();
        return this[ONERROR](er, entry)
      }
      fs$6.lstat(entry.absolute, (er, st) => {
        if (st && (this.keep || this.newer && st.mtime > entry.mtime)) {
          ...
          if (entry.type === 'Directory') {
            if (!this.noChmod && (!entry.mode || (st.mode & 0o7777) === entry.mode))
              ...
              fs$6.chmod(entry.absolute, entry.mode,
                er => this[MAKEFS](er, entry, done));
            }
          } else
            fs$6.rmdir(entry.absolute, er => this[MAKEFS](er, entry, done));
        } else
  ...
  [LINK] (entry, linkpath, link, done) {
    ...
    fs$6[link](linkpath, entry.absolute, er => {
      if (er)
        return this[ONERROR](er, entry)
      done();
      this[UNPEND]();
...
class UnpackSync extends Unpack$1 {
  [CHECKFS] (entry) {
    const er = this[MKDIR](path$5.dirname(entry.absolute), this.dmode, neverCalled);
    if (er)
      return this[ONERROR](er, entry)
    try {
      const st = fs$6.lstatSync(entry.absolute);
      if (this.keep || this.newer && st.mtime > entry.mtime)
        ...
        try {
          if (st.isDirectory()) {
            if (entry.type === 'Directory') {
              if (!this.noChmod && entry.mode && (st.mode & 0o7777) !== entry.mode)
                fs$6.chmodSync(entry.absolute, entry.mode);
            } else
              fs$6.rmdirSync(entry.absolute);
          } else
  ...
  [FILE] (entry, _) {
    ...
    let fd;
    try {
      fd = fs$6.openSync(entry.absolute, getFlag(entry.size), mode);
    } catch (er) {
      return oner(er)
    }
    const tx = this.transform ? this.transform(entry) || entry : entry;
    ...
    tx.on('data', chunk => {
      try {
        fs$6.writeSync(fd, chunk, 0, chunk.length);
      } catch (er) {
        oner(er);
      }
    });
    ...
    tx.on('end', _ => {
      ...
      if (entry.mtime && !this.noMtime) {
        ...
        const mtime = entry.mtime;
        try {
          fs$6.futimesSync(fd, atime, mtime);
        } catch (futimeser) {
          try {
            fs$6.utimesSync(entry.absolute, atime, mtime);
          } catch (utimeser) {
            er = futimeser;
          }
        }
      ...
      if (this[DOCHOWN](entry)) {
        ...
        try {
          fs$6.fchownSync(fd, uid, gid);
        } catch (fchowner) {
          try {
            fs$6.chownSync(entry.absolute, uid, gid);
          } catch (chowner) {
            er = er || fchowner;
          }
        }
  ...
  [DIRECTORY] (entry, _) {
    ...
    if (entry.mtime && !this.noMtime) {
      try {
        fs$6.utimesSync(entry.absolute, entry.atime || new Date(), entry.mtime);
      } catch (er) {}
    }
    if (this[DOCHOWN](entry)) {
      try {
        fs$6.chownSync(entry.absolute, this[UID](entry), this[GID](entry));
      } catch (er) {}
    }
    entry.resume();
  }
  ...
  [LINK] (entry, linkpath, link, _) {
    try {
      fs$6[link + 'Sync'](linkpath, entry.absolute);
      entry.resume();
    } catch (er) {
      return this[ONERROR](er, entry)
    }
...
const filesFilter = (opt, files) => {
  ...
  const mapHas = (file, r) => {
    const root = r || path$4.parse(file).root || '.';
    const ret = file === root ? false
      : map.has(file) ? map.get(file)
      : mapHas(path$4.dirname(file), root);
...
if (!fs$4[gracefulQueue]) {
  ...
  if (/\bgfs4\b/i.test(process.env.NODE_DEBUG || '')) {
    process.on('exit', function() {
      ...
      require$$0$3.equal(fs$4[gracefulQueue].length, 0);
    });
  }
}
...
function resetQueue () {
  var now = Date.now();
  for (var i = 0; i < fs$4[gracefulQueue].length; ++i) {
    // entries that are only a length of 2 are from an older version, don't
    // bother modifying those since they'll be retried anyway.
    if (fs$4[gracefulQueue][i].length > 2) {
      fs$4[gracefulQueue][i][3] = now; // startTime
      fs$4[gracefulQueue][i][4] = now; // lastTime
    }
  }
...
function retry () {
  ...
  if (fs$4[gracefulQueue].length === 0)
    return
...
var rimraf_1 = rimraf$2;
rimraf$2.sync = rimrafSync$2;
...
function rimraf$2 (p, options, cb) {
  if (typeof options === 'function') {
    cb = options;
    options = {};
  }
...
function rmkids(p, options, cb) {
  ...
  options.readdir(p, function (er, files) {
    ...
    files.forEach(function (f) {
      rimraf$2(path$2.join(p, f), options, function (er) {
        if (errState)
          return
        if (er)
          return cb(errState = er)
...
function rimrafSync$2 (p, options) {
  options = options || {};
  defaults(options);
  ...
  for (var i = 0; i < results.length; i++) {
    ...
    try {
      ...
      if (er.code === "ENOENT")
        return
      if (er.code === "EPERM")
        return isWindows$1 ? fixWinEPERMSync(p, options, er) : rmdirSync$1(p, options, er)
      if (er.code !== "EISDIR")
        throw er
...
function rmkidsSync (p, options) {
  ...
  options.readdirSync(p).forEach(function (f) {
    rimrafSync$2(path$2.join(p, f), options);
  });
...
function asyncMethod ( methodName ) {
	return function () {
		...
		return new Promise( function ( fulfil, reject ) {
			...
			fs$1[ methodName ].apply( fs$1, args );
		});
	};
}
...
function syncMethod ( methodName ) {
	return function () {
		var args = normaliseArguments( arguments );
		return fs$1[ methodName ].apply( fs$1, args );
	};
}
...
function createReadStream$1 () {
	...
	return fs$1.createReadStream( resolvedPath, options );
}
...
function createWriteStream$1 () {
	...
	mkdirp$1.sync( path$1.dirname( resolvedPath ) );
	return fs$1.createWriteStream( resolvedPath, options );
}
...
function exists$1 () {
	...
	return new Promise( function (fulfil) {
		fs$1.exists( target, function (exists$$1) { return fulfil( exists$$1 ); } );
	});
}

function existsSync$1 () {
	return fs$1.existsSync( resolvePath( arguments ) );
}
	...
	return function () {
		...
		return {
			to: function to () {
				...
				return new Promise( function ( fulfil, reject ) {
					mkdirp$1( path$1.dirname( dest ), function (err) {
						if ( err ) {
							reject( err );
						} else {
							fs$1[ methodName ]( src, dest, function (err) {
								if ( err ) {
									reject( err );
								} else {
									fulfil();
	...
	return function () {
		...
		return {
			to: function to () {
				...
				mkdirp$1.sync( path$1.dirname( dest ) );
				return fs$1[ methodName ]( src, dest );
			}
		};
	};
}
...
function open$1 () {
	...
	return new Promise( function ( fulfil, reject ) {
		function open$$1 () {
			fs$1.open( src, flags, options.mode, function ( err, fd ) {
				if ( err ) {
					reject( err );
				} else {
					fulfil( fd );
		...
		if ( /^[wa]/.test( flags ) ) {
			mkdirp$1( path$1.dirname( src ), function (err) {
				if ( err ) {
					reject( err );
				} else {
					open$$1();
...
function openSync$1 () {
	...
	// create dirs if necessary
	if ( /^[wa]/.test( flags ) ) {
		mkdirp$1.sync( path$1.dirname( src ) );
	}

	return fs$1.openSync( src, flags, options.mode );
}
			...
			return new Promise( function ( fulfil, reject ) {
				mkdirp$1( path$1.dirname( dest ), function (err) {
					if ( err ) {
						...
						fs$1.symlink( src, dest, options.type, function (err) {
							if ( err ) {
								reject( err );
							} else {
								fulfil();
...
function symlinkSync$1 () {
	...
	return {
		to: function to () {
			...
			var dest = ref.resolvedPath;
			mkdirp$1.sync( path$1.dirname( dest ) );
			return fs$1.symlinkSync( src, dest, options.type );
		}
	};
}
		...
		return new Promise( function ( fulfil, reject ) {
			mkdirp$1( path$1.dirname( dest ), function (err) {
				if ( err ) {
					...
					fs$1[ methodName ]( dest, data, opts, function (err) {
						if ( err ) {
							reject( err );
						} else {
							fulfil( data );
...
function syncMethod$2 ( methodName ) {
	return function () {
		...
		mkdirp$1.sync( path$1.dirname( dest ) );
		return fs$1[ methodName ]( dest, data );
	};
}
		...
		to: function to () {
			...
			function copydir ( src, dest, cb ) {
				mkdirp$1( dest, function (err) {
					...
					fs$1.readdir( src, function ( err, files ) {
						if ( err ) { return cb( err ); }
						...
						files.forEach( function ( filename ) {
							...
							fs$1.stat( srcpath, function ( err, stats ) {
								...
								readStream = fs$1.createReadStream( srcpath, readOptions );
								writeStream = fs$1.createWriteStream( destpath, writeOptions );
	...
	return {
		to: function to () {
			...
			function copydir ( src, dest ) {
				...
				fs$1.readdirSync( src ).forEach( function (filename) {
					var srcpath = src + path$1.sep + filename;
					var destpath = dest + path$1.sep + filename;

					if ( fs$1.statSync( srcpath ).isDirectory() ) {
						return copydir( srcpath, destpath );
					}

					var data = fs$1.readFileSync( srcpath, readOptions );
					fs$1.writeFileSync( destpath, data, writeOptions );
				});
			}
			...
			return new Promise( function ( fulfil, reject ) {
				mkdirp$1( path$1.dirname( dest ), function (err) {
					if ( err ) {
						...
						var readStream = fs$1.createReadStream( src, readOptions );
						var writeStream = fs$1.createWriteStream( dest, writeOptions );
...
function copyFileSync () {
	...
	return {
		to: function to () {
			...
			var data = fs$1.readFileSync( src, readOptions );

			mkdirp$1.sync( path$1.dirname( dest ) );
			fs$1.writeFileSync( dest, data, writeOptions );
		}
	};
}
...
var closeSync$1 = fs$1.closeSync;
var fchmodSync$1 = fs$1.fchmodSync;
var fchownSync$1 = fs$1.fchownSync;
var fstatSync$1 = fs$1.fstatSync;
var fsyncSync$1 = fs$1.fsyncSync;
var ftruncateSync$1 = fs$1.ftruncateSync;
var futimesSync$1 = fs$1.futimesSync;
var readSync$1 = fs$1.readSync;
...
sander_cjs.rmdirSync = rmdirSync;
...
sander_cjs.rimraf = rimraf$1;
sander_cjs.rimrafSync = rimrafSync$1;
...
var child_process = require$$0$a;
var URL = require$$7;
var Agent = dist$1;
var sander = sander_cjs;
var rimraf = rimraf_1$1;
...
const init = defineNuxtCommand({
  ...
  async invoke(args) {
    ...
    console.log(`
 \u{1F389}  Another ${superb.random()} Nuxt project just made! Next steps:` + [
      "",
      `\u{1F4C1}  \`cd ${rpath(dstDir)}\``,
      "\u{1F4BF}  Install dependencies with `npm install` or `yarn install`",
      "\u{1F680}  Start development server with `npm run dev` or `yarn dev`",
      ""
    ].join("\n\n     "));
  }
});
