...
const BUILTIN_MODULES$1 = new Set(module$1.builtinModules);
...
function fileURLToPath$1(id) {
  ...
  return normalizeSlash$1(url.fileURLToPath(id));
}
...
async function loadURL(url) {
  const code = await fs.promises.readFile(fileURLToPath$1(url), "utf-8");
  return code;
}
...
const extname$1 = function(p) {
  return path__default.posix.extname(normalizeWindowsPath$1(p));
};
const relative$1 = function(from, to) {
  return path__default.posix.relative(normalizeWindowsPath$1(from), normalizeWindowsPath$1(to));
};
const dirname$1 = function(p) {
  return path__default.posix.dirname(normalizeWindowsPath$1(p));
};
const format$1 = function(p) {
  return normalizeWindowsPath$1(path__default.posix.format(p));
};
const basename$1 = function(p, ext) {
  return path__default.posix.basename(normalizeWindowsPath$1(p), ext);
};
const parse$d = function(p) {
  return path__default.posix.parse(normalizeWindowsPath$1(p));
};
...
function read$1(jsonPath) {
  return find$1(path__default.dirname(jsonPath))
}
...
function find$1(dir) {
  try {
    const string = fs__default.readFileSync(
      path__default.toNamespacedPath(path__default.join(dir, 'package.json')),
      'utf8'
    );
    return {string}
  } catch (error) {
 ...
 */
function defaultGetFormat$1(url$1) {
  ...
  if (parsed.protocol === 'file:') {
    ...
    if (!format) {
      throw new ERR_UNKNOWN_FILE_EXTENSION$1(ext, url.fileURLToPath(url$1))
    }
 ...
 * @returns {void}
 */
function emitLegacyIndexDeprecation$1(url$1, packageJsonUrl, base, main) {
  const {format} = defaultGetFormat$1(url$1.href);
 ...
 * @returns {boolean}
 */
function fileExists$1(url$1) {
  return tryStatSync$1(url.fileURLToPath(url$1)).isFile()
}
...
function finalizeResolution$1(resolved, base) {
  ...
  if (stats.isDirectory()) {
    const error = new ERR_UNSUPPORTED_DIR_IMPORT$1(path, url.fileURLToPath(base));
...
function resolvePackageTargetString$1(
  ...
  if (pattern) return new url.URL(resolved.href.replace(patternRegEx$1, subpath))
...
function packageImportsResolve$1(name, base, conditions) {
  if (name === '#' || name.startsWith('#/')) {
    ...
    throw new ERR_INVALID_MODULE_SPECIFIER$1(name, reason, url.fileURLToPath(base))
  }
...
function packageResolve$1(specifier, base, conditions) {
  ...
  throw new ERR_MODULE_NOT_FOUND$1(packageName, url.fileURLToPath(base))
}
...
const DEFAULT_URL$1 = url.pathToFileURL(process.cwd());
...
function _resolve$1(id, opts = {}) {
  ...
  const realPath = fs.realpathSync(fileURLToPath$1(resolved));
...
const extname = function(p) {
  return path__default.posix.extname(normalizeWindowsPath(p));
};
const relative = function(from, to) {
  return path__default.posix.relative(normalizeWindowsPath(from), normalizeWindowsPath(to));
};
const dirname = function(p) {
  return path__default.posix.dirname(normalizeWindowsPath(p));
};
const format = function(p) {
  return normalizeWindowsPath(path__default.posix.format(p));
};
const basename = function(p, ext) {
  return path__default.posix.basename(normalizeWindowsPath(p), ext);
};
const parse$6 = function(p) {
  return path__default.posix.parse(normalizeWindowsPath(p));
};
...
exports.fileURLToPath = fileURLToPath$1;
...
exports.resolvePathSync = resolvePathSync$1;
exports.resolveSync = resolveSync$1;
