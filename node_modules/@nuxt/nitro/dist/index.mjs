...
function evalTemplate(ctx, input) {
...
function resolvePath(nitroContext, input, resolveBase = "") {
  ...
  if (nitroContext.externals) {
    rollupConfig.plugins.push(externals(defu(nitroContext.externals, {
      ...
      inline: [
        ...
        ...nitroContext.middleware.map((m) => m.handle).filter((i) => typeof i === "string"),
...
async function _build(nitroContext) {
  ...
  const rewriteBuildPaths = (input, to) => typeof input === "string" ? replaceAll(input, nitroContext.output.dir, to) : void 0;
...
export { build, createDevServer, generate, getNitroContext, prepare, resolveMiddleware, scanMiddleware, wpfs, writeTypes };
